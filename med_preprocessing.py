...
#     # print(target)
#     print(training_dataset_ohe[target].value_counts()[1])

params_dict_list = {'C': [0.005, 0.1, 1, 10], 'penalty': ['l1', 'l2', 'elasticnet'], 'l1_ratio': [0.5],
                    'class_weight':['balanced'], 'n_jobs': [-1], 'random_state':[42], 'max_iter':[100,200]}
...
for target in targets:
    ...
    model = LogisticRegression(class_weight='balanced', C=0.005, penalty='l2', max_iter=200, n_jobs=-1,random_state=42)
...
print('Saving topics stats...')
topics_dataset.to_excel(
    '/home/pavlychev1-ns_ca-sbrf-ru/notebooks/Doc2Vec/Doc2Vec_spr_experiments/' + 'exp_' + str(k) + '  ' + str(
        item) + '.xlsx')
...
#cозависимость болезней использовать
